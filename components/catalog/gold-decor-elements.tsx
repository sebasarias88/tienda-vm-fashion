...
export type SparkleEffectProps = {
  x: string
  y: string
  size: number
  delay: number
...
export function SparkleEffect({ x, y, size, delay, opacity }: SparkleEffectProps) {
...
export const GOLD_LINE_GRADIENT =
...
export type GoldBrushIntensity = 'hero' | 'subtle' | 'header'

type GoldBrushStrokesProps = {
  intensity?: GoldBrushIntensity
  showTopRight?: boolean
  showBottomAccent?: boolean
}

export function GoldBrushStrokes({
  intensity = 'hero',
  showTopRight = true,
  showBottomAccent = true,
}: GoldBrushStrokesProps) {
  ...
  const config = {
    hero: { tl: 0.15, tr: 0.18, bl: 0.1, wTL: 320, wTR: 380, wBL: 220 },
    subtle: { tl: 0.1, tr: 0.12, bl: 0.08, wTL: 240, wTR: 280, wBL: 160 },
    header: { tl: 0.1, tr: 0.12, bl: 0, wTL: 260, wTR: 300, wBL: 0 },
  }[intensity]
    ...
    <>
      ...
      >
        <svg
          viewBox="0 0 300 200"
          className="absolute left-0 top-0"
          style={{ width: config.wTL, opacity: config.tl, pointerEvents: 'none' }}
        >
          <path
...
export function GoldDecorLayer({
  className = '',
  children,
}: {
  className?: string
